...
function App() {
  ...
  const [recommendationSearchError,setRecommendationSearchError] = useState('')
  const[recommendLoader,setRecommendLoader] = useState(false)
  const [value,setValue] = useLocaleStorage(null,'moviejwt')
  
  return (
    <MovieContext.Provider value={{recentMovies,setRecentMovies,recommendedMovies,setRecommendedMovies,recommendationSearchError,setRecommendationSearchError,recommendLoader,setRecommendLoader,movieData,setMovieData,actorsData,setActorsData,setMovieError,movieError,reviewsData,setReviewsData,actorId,setActorId,setValue,value}}>
      <BrowserRouter>
        <Globalstyles/>
        <Navbar/>
        ...
        <Routes> 
          ...
          <Route path="/myactors" element={value?<MyActors/>:<Navigate to={'/login'}/>}/>
          <Route path="/mymovies" element={value?<MyMovies/>:<Navigate to={'/login'}/>}/>
          <Route path="/movie/:id" element={<Movie/>}/>
          <Route path="/login" element={<Login/>}/>
          <Route path="/signup" element={<Signup/>}/>
          <Route path="/actor/:name" element={<Actor/>} />
          <Route path ="/verification/:token" element={<SignupVerification/>}/>
          <Route path="/twittersentiment/:movie" element={<TwitterSentiment/>}/>
