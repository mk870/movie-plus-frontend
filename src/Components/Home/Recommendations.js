...
const Recommendations = () => {
  ...
  const recommendClickedMovie = (id)=>{
    ...
    postApiCall('https://movie-recommandation-api.herokuapp.com/recommend2',data,setRecommendationSearchError,setRecommendedMovies,setRecommendLoader,source)
  }
  return (
    <RecommendationStyles>
      ...
      <div className="searched-movie">
        <div className="searched_image">
          <img src={`https://image.tmdb.org/t/p/w500/${movieData[0].poster_path}`} alt='poster' />
        </div>
      ...
      <div className="recom-movies">
        {movieData.slice(1,9).map((movie,index)=>(
          <div className="card"key={index} onClick={()=>recommendClickedMovie(movie.id)}>
      ...
      {movieDataError && !movieData && !loader && <div className='error'>
        <h2>
          Failed to fetch data please check your network settings
        </h2>
      </div>}
    </RecommendationStyles>
  )
