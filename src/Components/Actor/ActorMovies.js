...
const ActorMovies = () => {
  ...
  return (
    <ActorMoviesStyles>
      {actorMovieList && !error && !loader && <div className='list'>
        <h3>Featured Movie-List</h3>
        <div className="card-list">
          {actorMovieList.map((movie,index)=>(
            <div className="item" key={index} onClick={()=>navigate(`/movie/${movie.id}`)}>
      ...
      {error !=='' && !loader && <ApiErrorPage error={`Network Error: failed to fetch ${name}'s movies`}/>}
