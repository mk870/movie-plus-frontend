...
const MyActors = () => {
  const url = 'https://movieplus-server.herokuapp.com/api/account/actors'
  ...
  const deleteItem = (id)=>{
    ...
    setActors('')
    setError('')
    DeleteApiCall(`https://movieplus-server.herokuapp.com/api/account/actor/${id}`,value,setOnDelete,setError,setLoader)
  }
  ...
  return (
    <MyActorsStyles>
      ...
      {!actors && error && !loader && <ApiErrorPage error={'Network Error: Sorry could not fetch your actors'}/>}
    </MyActorsStyles>
  )
}
