...
import { twitterSentimentApi } from "../../Components/Utils/BackEndApis";
...
const TwitterSentiment = () => {
  ...
  const wordCleaner = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
      arr2.push({
        text: arr[i],
        value: Number((Math.random() * 1000000).toFixed(0)),
      });
    }
    return arr2;
  };
  ...
  const msg =
    "Sorry no one is currently tweeting about this movie, please try another movie";
    ...
    <TwitterSentimentStyles colour={sentimentcolor(sentiments)}>
      ...
      {words && !error && sentiments && !loader && !noLiveTweets && (
        <div className="container">
          ...
          <p className="p-score">
            <p className="present-score">
              Current Twitter Sentiment Score is :
            </p>
      ...
      {!words && !error && !sentiments && !loader && noLiveTweets && (
        <ApiErrorPage error={msg} />
      )}
      {!words && error && !sentiments && !loader && !noLiveTweets && (
        <ApiErrorPage
          error={"Network error: Sorry could not fetch twitter sentiments"}
        />
      )}
